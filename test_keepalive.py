...
用法:
    python3 test_keepalive.py --duration 300  # 运行 5 分钟
"""
...
def log(msg):
    """记录日志"""
    line = f"[{time.strftime('%H:%M:%S')}] [test] {msg}"
...
def main():
    global running, public_url, start_time, duration
    
    parser = argparse.ArgumentParser(description="测试保活脚本")
    parser.add_argument('--duration', type=int, default=300, help='运行时长（秒），默认 300 秒（5 分钟）')
